...
PROD_REPO=${CLOUDSMITH_PROD_REPO:-'opencpn/vhf-info-prod'}
BETA_REPO=${CLOUDSMITH_BETA_REPO:-'opencpn/vhf-info-beta'}
ALPHA_REPO=${CLOUDSMITH_ALPHA_REPO:-'opencpn/vhf-info-alpha'}
...
if [ -n "${OCPN_TARGET}" ]; then
    tarball_name=vhf_info_pi-0.4.8.0-${PKG_TARGET}-x86_64-${PKG_TARGET_VERSION}-${OCPN_TARGET}-tarball
else
    tarball_name=vhf_info_pi-0.4.8.0-${PKG_TARGET}-x86_64-${PKG_TARGET_VERSION}-tarball
fi
...
if [ "$BUILD_BRANCH_LOWER" = "master" ]; then
    ...
    if [ -n "$BUILD_TAG"  ]; then
        ...
        VERSION="0.4.8.0+${BUILD_ID}.${commit}"
...
# Only upload if the owner did the pull request. Non-owners probably don't have security to upload to the cloudsmith repository
    if [ "$CIRCLE_PROJECT_USERNAME" = "$CIRCLE_USERNAME" ]; then
        cloudsmith push raw --republish --no-wait-for-sync \
            --name vhf_info_pi-0.4.8.0-ubuntu-x86_64-22.10-${OCPN_TARGET}-metadata \
            --version ${VERSION} \
            --summary "vhf_info_pi opencpn plugin metadata for automatic installation" \
            $REPO $xml

        cloudsmith push raw --republish --no-wait-for-sync \
            --name $tarball_name \
            --version ${VERSION} \
            --summary "vhf_info_pi opencpn plugin tarball for automatic installation" \
            $REPO $tarball

        if [ "${PKG_EXT}" != "gz" ] && [ "$pkg" != "" ]; then
            cloudsmith push raw --republish --no-wait-for-sync \
                --name opencpn-package-vhf_info_pi-0.4.8.0-ubuntu-x86_64-22.10-${OCPN_TARGET}.${PKG_EXT} \
                --version ${VERSION} \
                --summary "vhf_info_pi .${PKG_EXT} installation package" \
                $REPO $pkg
        fi
    fi
fi
